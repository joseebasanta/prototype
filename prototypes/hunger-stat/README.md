...
1. **Odometer** — the number `795` rolls up, slot-machine style: each digit is a masked
...
3. **Hand-off** — at ~3s the whole `795 / million people` group slides up and fades out while,
...
<StatReveal />                       // one-shot: 795 → "are going hungry."
<StatReveal loop />                  // replay forever (used by the preview)
<StatReveal count="200" unit="Builders" finalLead="Forma " finalEmph="parte" />
```
...
| --------------- | --------- | ------------------ | -------------------------------------------------------------- |
| `count`         | `string`  | `"795"`            | Number the odometer rolls up to. Digits roll; other chars static. |
